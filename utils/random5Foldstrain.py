...
def manyTimesTrain(model,firstFolds, times, modelName="mobileNet",load_model=False):
    ...
    :param firstFolds: first partition (format shoubld be r"...../dataSetName/1" 1 means the times of which the number of partition
    ...
    for i in range(times):
        ...
        originalPath = firstFolds #firstFolds[:-1] + str(int(firstFolds[-1]) + i)
...
if __name__ == '__main__':
    seed_torch(2021)
    model = "mobileLarge"
    makeCrossValidation = MakeCrossValidationDataFolder(3, 50)
    cvFirstPath = makeCrossValidation.begin_split()

    manyTimesTrain(model, cvFirstPath, times=3, modelName="mobileBigBig")
